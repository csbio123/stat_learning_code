...
prep_data<-function(covars, sva_file, bmi_sv_file, hba_sv_file, pheno ){
  test.obj2<-lapply(covars,`[`, c("ChipID", "PC1", "PC2", "Age.norm", "hba1c.catg"))#DELETE FOLLOWING VARS AS APPROPRIATE (ACCORDING TO TRAIT)
...
prepare_feature_sets<-function(data_list, outptut_dir, prefix="dataset") {
  ...
  gene_expression =data_list$hba_enet[,61:ncol(data_list$hba_enet)]
  
  technical_cofounders = subset(data_list$hba_enet, select=c(sv1, sv2, sv3, sv4, sv5, sv6, sv7, sv8, sv9, sv10, sv11, sv12, sv13, sv14, sv15,BeadChips_barcode9234985067, BeadChips_barcode9234985072, BeadChips_barcode9234985077, BeadChips_barcode9234985098, BeadChips_barcode9249896009, BeadChips_barcode9249896020, BeadChips_barcode9249896033, BeadChips_barcode9249896043, BeadChips_barcode9249896074, BeadChips_barcode9249896075, BeadChips_barcode9249896076, BeadChips_barcode9249896081, BeadChips_barcode9249896087, BeadChips_barcode9249896089, BeadChips_barcode9249896093, BeadChips_barcode9249896098, BeadChips_barcode9250939027, BeadChips_barcode9252619016, BeadChips_barcode9252619022, BeadChips_barcode9252619073, BeadChips_barcode9252619074, BeadChips_barcode9252619076, BeadChips_barcode9252619085, BeadChips_barcode9257302086,loading_wellB, loading_wellC, loading_wellD, loading_wellE, loading_wellF, loading_wellG, loading_wellH, loading_wellI, loading_wellJ, loading_wellK, loading_wellL))
  clinical_cofounders = subset(data_list$hba_enet, select=c(BMI.norm))
  imputed_data = data_list$imputed_sets
  personal_cofounders = subset(data_list$hba_enet, select=c(MaleGender)) #decided to use gender alongside SVs4&5 as these SVs are more technical than demographic
  ...
  sapply(seq(1, length(imputed_data)), function(x) {
    ...
    features = cbind(imp_d, personal_cofounders, clinical_cofounders, technical_cofounders, gene_expression)
    print(dim(features))
...
prepared_data = prepare_feature_sets(data_list, outptut_dir = "/users/spjtcoi/brc_scratch/project_tomi/conrad/reanalyse/drug_naive/new_protocol_25thOct/PREDICTION/validation_input/models_using_chipID_well/hba")
#prepared_data = prepare_feature_sets(data_list, outptut_dir = "C:/Users/spjtcoi/Google Drive/conrad/dataset2_chronic_scz/inputfiles_for_validation/hba3/")
names(prepared_data)
