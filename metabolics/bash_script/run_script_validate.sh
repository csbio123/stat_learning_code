...
input_dir=/users/spjtcoi/git/stat_learning_code/metabolics/validation_prediction_new/not_normalized_data
...
feature_list=${input_dir}/../features_shared_Training_Validation.csv
...
output_dir=/users/spjtcoi/brc_scratch/project_tomi/conrad/reanalyse/drug_naive/new_protocol_25thOct/prediction_output_started_11-02-18/non_normalised/transcript_only
...
mkdir -p $output_dir
Rscript /users/spjtcoi/git/stat_learning_code/metabolics/predictions/validation_classification_prediction_tomi.R $training_input $test_input $training_GE $test_GE $feature_list $remove_feature_list  $output_dir> $output_dir/test.${SGE_TASK_ID}.out 2> $output_dir/test.${SGE_TASK_ID}.err
