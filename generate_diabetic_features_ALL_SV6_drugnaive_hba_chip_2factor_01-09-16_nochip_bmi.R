...
prep_data<-function(covars, sva_file, bmi_sv_file, hba_sv_file, pheno, tpheno ){
  test.obj2<-lapply(covars,`[`, c("ChipID", "PC1", "PC2", "Age.norm", "BMI.catg", "med.days"))#DELETE FOLLOWING VARS AS APPROPRIATE (ACCORDING TO TRAIT)
...
prepare_feature_sets<-function(data_list, outptut_dir, prefix="dataset") {
  ...
  technical_cofounders = subset(data_list$bmi_enet, select=c(Timepoint, sv1, sv2, sv3, sv4, sv5, sv6))
...
prepared_data = prepare_feature_sets(data_list, outptut_dir = "/users/spjtcoi/brc_scratch/project_tomi/conrad/reanalyse/drug_naive/new_protocol_25thOct/IMPUTATION_SETS/input_files_BMI_2catg_nochips/")
names(prepared_data)
